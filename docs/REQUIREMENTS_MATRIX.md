...
| R-031 | Final report is JSON | Report | Mandatory | `json.dumps`-serializable report builders (self-play + MCP-backed with local status fields) | `orchestration/report.py`, `mcp_client/game_report.py`, `tests/unit/mcp_client/test_game_report.py` | `json.dumps(report)` succeeds; tokens absent | Med | In progress |
