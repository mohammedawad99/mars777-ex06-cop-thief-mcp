...
| R-002 | Two MCP servers exist: one for Cop, one for Thief | MCP | Mandatory | Independent FastMCP cop/thief builders + run entrypoints on separate ports | `mcp_servers/cop_server.py`, `mcp_servers/thief_server.py`, `mcp_servers/run_*.py`, `tests/unit/mcp_servers/` | Tests assert both build with expected tools | Med | In progress |
...
| R-004 | MCP communication is HTTP-based from the beginning (not stdio-only) | MCP | Mandatory | FastMCP `transport="http"` builders + run entrypoints (ADR-0020) | `config/mcp.local.default.json`, `mcp_servers/run_*.py`, `DECISIONS.md` ADR-0020 | Servers run on HTTP locally; tests build http app | Med | In progress |
...
| R-010 | URLs are protected by token/auth; no open public endpoint | Security | Mandatory | Protected tools require `auth_token` checked vs env var (local dev auth, ADR-0021); upgrade to request-metadata for cloud | `mcp_servers/auth.py`, `mcp_servers/tools.py`, `tests/unit/mcp_servers/test_auth.py` | Tests: matching token ok, missing/wrong rejected, token never revealed | High | In progress |
