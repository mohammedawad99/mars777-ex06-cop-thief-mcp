...
| R-011 | Game is turn-based | Game | Mandatory | Engine advances one actor per turn deterministically | Engine state log | Unit test alternates turns | Low | Planned |
...
| R-016 | Each sub-game has up to 25 moves | Game | Mandatory | `max_moves` default 25 enforced by engine | Config, engine guard | Sub-game caps at 25 moves | Low | Done |
| R-017 | Cop catches Thief by reaching the same cell | Game | Mandatory | Capture = coordinate equality check | Engine capture logic | Unit test: same cell → capture | Low | Planned |
| R-018 | Thief wins a sub-game by surviving all moves without capture | Game | Mandatory | Survival-to-`max_moves` → Thief win | Engine terminal logic | Unit test: no capture → Thief win | Low | Planned |
| R-019 | Movement allows one step in all 8 directions incl. diagonals | Game | Mandatory | `movement_directions` = 8 compass dirs | Config, movement rules | Unit test each direction | Low | Done |
| R-020 | Stay not allowed in baseline unless explicitly negotiated/documented | Game | Mandatory | `allow_stay` default `false`; toggle documented | Config, protocol doc | Engine rejects stay when false | Low | Done |
| R-021 | Cop may place barriers instead of moving | Game | Mandatory | Cop action set includes `place_barrier` | Engine action model | Unit test: barrier as a turn | Med | Planned |
| R-022 | Cop may place at most 5 barriers per sub-game | Game | Mandatory | `max_barriers` default 5 enforced per sub-game | Config, engine counter | Unit test: 6th barrier rejected | Low | Planned |
| R-023 | Thief cannot place barriers | Game | Mandatory | Thief action set excludes barrier placement | Engine role guard | Unit test: thief barrier illegal | Low | Planned |
| R-024 | Barriers block both Cop and Thief | Game | Mandatory | Occupancy/collision checks include barriers | Engine collision logic | Unit test: move into barrier blocked | Low | Planned |
| R-025 | Illegal moves are detected and logged | Game | Mandatory | Validator rejects + structured log entry | Logs, validator tests | Unit test: illegal move logged | Med | Planned |
...
| R-027 | Scoring — Cop win: Cop 20, Thief 5 | Game | Mandatory | `scoring.cop_win=20`, `cop_loss/thief_loss=5` | `config/game.default.json` | Unit test scoring on cop win | Low | Done |
| R-028 | Scoring — Thief win: Thief 10, Cop 5 | Game | Mandatory | `scoring.thief_win=10`, `cop_loss=5` | Config | Unit test scoring on thief win | Low | Done |
