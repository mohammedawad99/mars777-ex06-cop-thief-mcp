...
| Bonus mutual agreement | **pending** — `mutual_agreement: false`, `partner_confirmation_status: pending`; validator blocks a premature `true`; partner must confirm the `result_hash` matches | Deferred (Stage 15E) |
...
| Bonus report sent to partner/lecturer | **not sent** — handoff prepared for the partner; no Gmail sent this stage | Deferred (Stage 15E) |
