...
| R-007 | Inter-group play with another team is supported | Bonus | Bonus-Mandatory | Readiness gate + a tested adapter for partner orcai-mj's `setup`/`observe`/`my_move`/`state` contract (0-based `[row,col]`, 5x5/8x8, thief-first); readiness/smoke script. Self cloud ready; partner URLs/tokens + live arg-schema confirmation still pending | `scripts/bonus_partner_readiness.py`, `scripts/bonus_interop_smoke.py`, `bonus/intake.py`, `bonus/partner_adapter.py`, `INTERGROUP_BONUS_PROTOCOL.md` | Readiness gates exit 0; smoke/`bonus_ready` flip true only after a real partner smoke | High | In progress |
