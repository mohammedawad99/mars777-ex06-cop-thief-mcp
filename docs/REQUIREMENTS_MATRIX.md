...
| R-035 | If two groups disagree, bonus may be rejected | Bonus | Bonus-Mandatory | `mutual_agreement`/`partner_confirmation_status` gate the claim; validator **rejects** `mutual_agreement=true` until `confirmed`; partner handoff carries `result_hash` for cross-check | `bonus/bonus_handoff.py`, `reporting/validators.py`, `results/evidence/bonus_game_partner_handoff.example.json`, `tests/unit/bonus/` | Premature agreement blocked by validator (tested); handoff hash matches report | Med | Done (mechanism); confirmation pending |
