...
| R-005 | Baseline supports local self-play | Game | Mandatory | Full local self-play now runs **over MCP/HTTP** each turn (orchestrator + role servers), plus the in-process backbone | `mcp_client/game_flow.py`, `mcp_client/game_smoke.py`, `tests/integration/mcp/test_http_game_e2e.py` | 6-sub-game game smoke passes over HTTP (exit 0) | Med | Done |
| R-006 | Submission supports cloud/self-play through public URLs | Cloud | Mandatory | Cloud **packaging + preflight** ready (Dockerfile, role entrypoint, Cloud Run config); live deploy gated/manual — **not deployed yet** | `Dockerfile`, `mcp_servers/cloud_entrypoint.py`, `config/cloud.default.json`, `deployment/preflight.py`, `docs/CLOUD_DEPLOYMENT.md` | Preflight `status: ok`; live URLs pending deploy | High | In progress |
...
| R-009 | Each team exposes two URLs: Cop MCP URL and Thief MCP URL | Cloud | Mandatory | Two role-separated services packaged (`mars777-cop-mcp`/`mars777-thief-mcp`); URL placeholders `<set-after-deployment>` until deployed | `config/cloud.default.json`, `mcp_servers/cloud_entrypoint.py`, reports `cop_mcp_url`/`thief_mcp_url` | Two services build; real URLs pending deploy | Med | In progress |
...
| R-033 | Internal report includes group name/code, students, repo, Cop URL, Thief URL, timezone, sub_games, totals | Report | Mandatory | Official internal schema carries all fields incl. students + cop/thief MCP urls; required-field validation | `reporting/schemas.py`, `reporting/official_report.py`, `tests/unit/reporting/test_official_report.py` | Required-field check on built report | Med | Done |
| R-034 | Bonus report includes both group names, both repos, four URLs, students, sub_games, totals_by_group, bonus_claim, mutual_agreement | Bonus | Bonus-Mandatory | Bonus schema example with all fields (no real run claimed); validated | `reporting/schemas.py`, `reporting/official_report.py`, `tests/unit/reporting/` | Schema validation + field check; `bonus_claim=false` | High | In progress |
...
| R-037 | Use Gmail API or another documented method; Google API preferred | Email | Mandatory | Gmail API client libs; dry-run default + live-gated send (`RUN_GMAIL_LIVE=1`); minimal `gmail.send` scope; OAuth files outside repo | `gmail/sender.py`, `gmail/auth.py`, `gmail/send_report.py`, `tests/unit/gmail/` | Dry-run exit 0; mocked send maps message id; live not run | Med | In progress |
