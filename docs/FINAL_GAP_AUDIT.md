...
| Bonus_game Gmail report | **draft/preview only** — JSON-only bonus_game draft to the lecturer created in Gmail and **not sent** (`gmail_draft_created: true`, `live_gmail_sent: false`, `bonus_email_sent: false`); body is ID-redacted; `RUN_GMAIL_LIVE` never set | Done (draft); live send deferred |
...
| Bonus report sent live to partner/lecturer | **not sent** — partner handoff prepared and a lecturer Gmail **draft** created (Stage 15E), but no live email sent | Deferred (Stage 15F) |
