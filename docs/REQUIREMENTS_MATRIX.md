...
| R-006 | Submission supports cloud/self-play through public URLs | Cloud | Mandatory | Cloud **packaging + preflight** ready (Dockerfile, role entrypoint, Cloud Run config); **live-readiness preflight** (read-only) checks deploy prerequisites and lists blockers; live deploy gated/manual — **not deployed yet** | `Dockerfile`, `mcp_servers/cloud_entrypoint.py`, `config/cloud.default.json`, `deployment/preflight.py`, `deployment/live_readiness.py`, `deployment/gcloud_checks.py`, `docs/CLOUD_DEPLOYMENT.md` | Preflight `status: ok`; live-readiness exits 0 with blockers listed; live URLs pending deploy | High | In progress |
