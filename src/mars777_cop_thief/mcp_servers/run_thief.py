"""Local Thief MCP server entrypoint (HTTP).

Run locally with:

    uv run python -m mars777_cop_thief.mcp_servers.run_thief
...
def main() -> None:  # pragma: no cover - starts a long-running server
    ...
    server = build_thief_server(game_config, mcp_config)
    host, _, path = server_http_settings(mcp_config["thief_server"])
    port = resolve_port(mcp_config["thief_server"], "THIEF_MCP_PORT")
