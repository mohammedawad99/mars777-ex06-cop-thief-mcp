...
| R-007 | Inter-group play with another team is supported | Bonus | Bonus-Mandatory | Readiness gate ingests a local partner file, validates intake + partner URLs, and runs a partner compatibility smoke; protocol doc; bonus schema. Self cloud is ready; partner data still pending | `scripts/bonus_partner_readiness.py`, `bonus/intake.py`, `INTERGROUP_BONUS_PROTOCOL.md`, `results/evidence/bonus_readiness.example.json` | Readiness gate exits 0; `bonus_ready` flips true only after a real partner smoke | High | In progress |
